...
- **Imports of DRF and of the HTTP views from outside the transport are ratcheted.** `tests/test_import_boundaries.py` lists every module outside `views/`, `viewsets/` and `openapi/` that imports `rest_framework` or `rest_framework_services.views`, and what it imports. An unlisted import fails, and so does a listed one that no longer exists, so both lists can only shrink. Most entries are legitimate — the keyword-pool resolver still lives in `views/utils.py` — but the dispatch core's transport neutrality is only as true as these imports, so a new one is a decision to write down beside the entry, never a side effect. When you move something *out* of `views/`, delete its entries in the same change.
...
- **Don't invent a new filtering DSL.** For list endpoints DRF's `filter_backends` are the default. The one blessed spec-level hook is `SelectorSpec.filter_set` — a `django-filter` `FilterSet` applied by duck typing (`(data, queryset) -> .qs`, so `types/` and the package import nothing), there for *retrieve* selectors and transport-neutral declaration; it **replaces** `DjangoFilterBackend` on the list path (guarded at `as_view()` in `views.spec_validation.validate_filter_set_no_backend_conflict`). When a selector returns an aggregate / computed object rather than a queryset, those `?param` values are computation inputs — use `kwargs` / `get_selector_kwargs()`. See `docs/recipes/selector-filtering.md`.
...
- **Adding a view that dispatches a `ServiceSpec`?** Don't reassemble the kwargs pool or call `dispatch_spec` directly — go through `views.mutation.utils.dispatch_mutation_for_spec` (mutations) or `selectors.utils.dispatch_selector_for_spec` (selectors). Both resolve the view's hook chains into a `ViewHooks` carrier and hand off to the one `dispatch_spec` core, so the kwargs-resolution chain (`spec.kwargs` → `get_<action>_*_kwargs` → `get_*_kwargs`) stays consistent and the spec's own providers resolve exactly once. **A behaviour that belongs to the spec belongs in the core** — put it in the view layer and it is honoured over HTTP and silently skipped over MCP; `tests/test_transport_parity.py` is the standing check. Its sibling `tests/test_sync_async_parity.py` guards the other axis — the sync core against `adispatch_spec` — by driving one case through both and comparing outcomes, including the signatures of every sync/async pair. Anything you change in one core belongs in a case there. Also hook fail-fast validation into `as_view()` via `views.spec_validation.validate_mutation_view_spec` / `validate_selector_view_spec`.
