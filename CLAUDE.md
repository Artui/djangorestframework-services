...
- **Adding a view that dispatches a `ServiceSpec`?** Don't reassemble the kwargs pool or call `dispatch_spec` directly — go through `views.mutation.utils.dispatch_mutation_for_spec` (mutations) or `selectors.utils.dispatch_selector_for_spec` (selectors). Both resolve the view's hook chains into a `ViewHooks` carrier and hand off to the one `dispatch_spec` core, so the kwargs-resolution chain (`spec.kwargs` → `get_<action>_*_kwargs` → `get_*_kwargs`) stays consistent and the spec's own providers resolve exactly once. **A behaviour that belongs to the spec belongs in the core** — put it in the view layer and it is honoured over HTTP and silently skipped over MCP; `tests/test_transport_parity.py` is the standing check. Also hook fail-fast validation into `as_view()` via `views.spec_validation.validate_mutation_view_spec` / `validate_selector_view_spec`.
