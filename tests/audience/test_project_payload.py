...
class _Invoice(serializers.Serializer):
    id = serializers.UUIDField(style={AGENT: AgentField.handle()})
...
def test_drops_hidden_and_speaks_labels() -> None:
    projected = project_payload(PAYLOAD, build_agent_projection(_Invoice))
...
def test_projects_a_list_of_records() -> None:
    projected = project_payload([PAYLOAD, PAYLOAD], build_agent_projection(_Invoice))
...
def test_empty_projection_returns_payload_unchanged() -> None:
    ...
    payload = {"name": "x"}
    assert project_payload(payload, build_agent_projection(_Plain)) is payload
