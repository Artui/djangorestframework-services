"""Tests for ``paginate_output`` and the ``OutputPage`` it returns.
...
def test_the_envelope_carries_rendered_rows_not_the_slice() -> None:
    ...
    page = paginate_output(ROWS, limit=2)
...
def test_a_queryset_is_counted_not_materialized() -> None:
    ...
    page = paginate_output(Author.objects.order_by("id"), page=2, limit=2)
