...
class RelationPhase(IntEnum):
    ...
    :class:`~rest_framework_services.types.relation_spec.RelationSpec`) and the
    driver walks the phases in this enum's order, never the mapping's:

    1. :attr:`FORWARD` — the FK column lives on the parent, so the target row
    ...
    2. *the parent's* ``save()`` — not a phase; the boundary the phases are
       named around.
    3. :attr:`REVERSE` — the FK column lives on the other row (reverse FK
       collections and reverse one-to-ones), so the parent must have a primary
       key to point at.
    4. :attr:`GENERIC` — generic relations, which need the saved parent's
       content type *and* primary key.
    5. :attr:`M2M` — through-table writes, which need both rows saved.
